...
def load(path, alternative):
    try:
        with codecs.open(_path('_'+path+'.bin'), 'rb') as f:
            return pickle.load(f)
    except:
        with codecs.open(_path('_'+path+'.bin'), 'wb') as f:
            out = alternative()
            pickle.dump(out, f)
            return out
...
UNIHAN = load('unihan', get_unihan)
GY = load('guangyun', get_gy)
GBK = load('gbk', get_gbk)
BIG5 = load('big5', get_big5)
TLS = load('tls', get_tls)
tone_converter = dict(zip(
